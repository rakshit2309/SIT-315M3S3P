...
int main(int argc, char **argv) {
    ...
    // Initialize input vectors and output vector
    init(v1, SZ);
    init(v2, SZ);
    init(v_out, SZ); 
    ...
    // Setup memory buffers for kernel arguments
    setup_kernel_memory();
...
void print(int *A, int size)
{
    // Function to print array elements for debugging
    if (PRINT == 0)
    {
        return;
    }
...
void free_memory() {
    // Free OpenCL resources and allocated memory
...
void setup_kernel_memory() {
    // Create OpenCL buffers and copy data from host to device
...
cl_device_id create_device() {
    // Create OpenCL device (GPU or CPU)
